...
def genetic_algorithm_schedule(driver_list, shift_duration, num_routes, selected_day, text_widget, break_time=10, min_break_time=30, 
    ...
    for _ in range(population_size):
        ...
        population.append((schedule, score))
    ...
    for gen in range(generations):
        ...
        if population[0][1] > best_score:
            ...
            best_schedule = population[0][0]
            no_improvement_count = 0
        else:
            no_improvement_count += 1

        if best_score == num_routes:
            text_widget.delete(1.0, END)
    ...
    text_widget.delete(1.0, END)
    if best_schedule and best_score > 0:
        df = pd.DataFrame(best_schedule)
...
def generate_genetic_schedule_A():
    try:
        ...
        genetic_algorithm_schedule(drivers_A, shift_duration_A, num_routes, selected_day, schedule_text, generations=10, population_size=5)
    except ValueError:
        schedule_text.insert(END, "\nОшибка: Введите корректное количество маршруток\n")
...
def generate_genetic_schedule_B():
    try:
        ...
        genetic_algorithm_schedule(drivers_B, shift_duration_B, num_routes, selected_day, schedule_text, generations=10, population_size=5)
    except ValueError:
        schedule_text.insert(END, "\nОшибка: Введите корректное количество маршруток\n")
...
def generate_genetic_schedule_AB():
    try:
        ...
        genetic_algorithm_schedule(combined_drivers, shift_duration_for_combined, num_routes, selected_day, schedule_text, generations=10, 
        population_size=5)
    except ValueError:
        schedule_text.insert(END, "\nОшибка: Введите корректное количество маршруток\n")
